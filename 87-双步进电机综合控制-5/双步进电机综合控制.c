...
unsigned int speed;
int	dis_data;	
static unsigned int  Kp,KI,KD,Ti,Td;//自行定义Kp,Ti,Td;; //比例常数Proportional Const
double  Ki,Kd,T; 
double error=0,error_1=0,error_2=0; //Error[-1]
double E0,E1=0,E2=0; //设定目标Desired Value 周期T先设为1ms					//变量
double P_now=0,P_last1=0,P_last2=0;
double Xjiaodu,Yjiaodu,Zjiaodu;
unsigned char num;
...
Kp=5;
Ti=10;
Td=0;//自行定义Kp,Ti,Td //比例常数Proportional Cons
T=0.001; 
	while(1)
	{
/* if(times1==1)
...
Yjiaodu=GetData(ACCEL_YOUT_H);
Zjiaodu=GetData(ACCEL_ZOUT_H);
Yjiaodu=Yjiaodu/64;
Zjiaodu=Zjiaodu/64;
